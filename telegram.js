...
gottfrid_bot.on('message', () => {
    ...
    // parse_list(messages)
    // let value = []
    //if(  (messageUpdateLast+messageUpdateTimeOut)<Date.now()){
    //console.log("message :" + message)
    messageUpdateLast = Date.now(); //eventArray.queueAdd(value[0]);
    //queryMaria.insertQuery
    //(update[0].message.from.id,update[0].message.from.first_name,update[0].=>
    //message.from.last_name ,update[0].message.from.username)   
    //TODO Добавить в массив событий объект события разного уровня приоритетности добавить в core очередь     
...
gottfrid_bot.onText(/\/reminder(.+) at ([01]?[0-9]|2[0-3]):([0-5]?[0-9])/, (message, match) => {
    ...
    //FIXME подключить к базе и создать таблицу Remind
...
gottfrid_bot.onText(/\/chirp/, message => {
    const msgId = message.message_id;
    gottfrid_bot.deleteMessage(message.chat.id, msgId);
    gottfrid_bot.sendMessage(message.chat.id, 'CHIIIIIIIIIIIIIIIIIRP CHIIIIIIIIIIIIRP');
});
...
gottfrid_bot.onText(/\/setGlobalRank \@([A-Za-z0-9]+[A-Za-z0-9/_]*) ([0-9])/gi, (msg, match) => {
    ...
    link.setGlobalRank(match[1], match[2], usrId);
})
...
gottfrid_bot.onText(/\/whoami/, (msg) => {
        let str = "";
